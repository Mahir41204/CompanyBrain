...
function severityClass(value) {
  ...
  if (["critical", "high", "expired"].includes(normalized)) return "high";
  if (["medium", "stale", "aging"].includes(normalized)) return "medium";
  return "low";
}
  ...
  $("overviewKpis").innerHTML = [
    {
      ...
      delta: `${kpis.knowledge_risk_score.direction === "up" ? "+" : ""}${kpis.knowledge_risk_score.delta} points this month`,
      cls: kpis.knowledge_risk_score.value >= 75 ? "bad" : "warn"
    },
    { label: "Nodes", value: kpis.nodes, delta: "Mapped memory" },
    { label: "Relations", value: kpis.relations, delta: "Typed dependencies" },
    { label: "Coverage", value: pct(kpis.coverage), delta: "Operational estimate", cls: "warn" },
    { label: "Conflicts", value: kpis.conflicts, delta: "Need resolution", cls: kpis.conflicts ? "bad" : "" }
  ].map((item) => `
    <article class="kpi">
      <div class="label">${item.label}</div>
      <div class="kpi-value">${item.value}</div>
      <div class="delta ${item.cls || ""}">${item.delta}</div>
    </article>
  `).join("");
...
function renderRows(id, rows, renderer) {
  $(id).innerHTML = rows.length ? rows.map((row) => `<div class="row">${renderer(row)}</div>`).join("") : `<div class="row-meta">No items</div>`;
}
...
function renderScenarioControls(library) {
  ...
  $("scenarioType").innerHTML = library.map((item) => `<option value="${item.type}">${item.label}</option>`).join("");
  $("scenarioLibrary").innerHTML = library.map((item) => `
    <button data-scenario="${item.type}" data-target="${escapeHtml(item.default_target)}">${escapeHtml(item.label)} · ${escapeHtml(item.default_target)}</button>
  `).join("");
...
async function boot() {
  try {
    ...
    await loadSimulation({ type: "tool_outage", target: "Zendesk", mitigations: [] });
    setOnline(true, "Live");
  } catch (error) {
